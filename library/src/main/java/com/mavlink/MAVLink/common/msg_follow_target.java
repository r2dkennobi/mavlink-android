...
public class msg_follow_target extends MAVLinkMessage{
    ...
    */
    public MAVLinkPacket pack(){
        ...
        for (float aVel : vel) {
            packet.payload.putFloat(aVel);
        }


        for (float anAcc : acc) {
            packet.payload.putFloat(anAcc);
        }


        for (float anAttitude_q : attitude_q) {
            packet.payload.putFloat(anAttitude_q);
        }


        for (float rate : rates) {
            packet.payload.putFloat(rate);
        }


        for (float aPosition_cov : position_cov) {
            packet.payload.putFloat(aPosition_cov);
        }
